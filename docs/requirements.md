...
## 追補：販売・マルチテナント要件（2026-09-15 追記）

本システムを「完成版」として複製・サブスク販売する構想に伴う要件。詳細設計は別ドキュメント参照。

### ビジネスモデル（オーナー確定）
- 基本は **空で納品**（システムのみ提供、在庫データは入れない）。顧客が自分で在庫を入力。
- オプション: 顧客がExcel/CSV/PDFを持つ場合、**初回データ登録をオーナー＋エージェントが代行（有償）**。
- 納品後の追加データも **別途費用でエージェント代行**。

### 在庫の分離要件（フェーズ2で実装済み）
- カテゴリー内は同一品番で在庫共有可、**カテゴリーを跨いだ共有はしない（独立）**。
- 現行の共有動作も残し、**shared/perCategory を運営者が切替**（運営者ページ /admin/owner）。
- 顧客管理者は在庫モードを変更できない（設定ミス事故の防止）。

### 販売方式（フェーズ3・方針確定「いいとこ取り」）
- **コードはマルチテナント対応**（tenantId で全テナント分離、修正は1回で全反映）。
- **デプロイは選べる**：共有インスタンス（相乗り） or 専用インスタンス（顧客ごとに別DB/別アプリ）。
- 権限階層: **master(運営者) > admin(各テナント管理者) > user(閲覧)**。

### FK2（Honda Civic Type R）パーツデータ要件（2026-09-17〜18 実装済み）
外部サイト parts-honda.uk からの純正パーツ取込に関する要件。

- **LH / RH は別カテゴリー**として管理（`ABA-FK2 (LH)` / `ABA-FK2 (RH)`）。同名カテゴリーのため
  DB操作は必ず categoryId で区別する。左右で図面コード・品番が異なる部品（ワイパー/ダッシュ/
  ペダル/ハーネス/ボンネット/ステアリング/A-C等）があり、RHはRHサイトの図面一覧に従う。
- **ジャンル名・部品名・ユニット名は英数字のまま**（翻訳しない）。ジャンルは機能別（GT3-049体系準拠）。
- **価格は元通貨（EUR）で保存**し、表示時に当日レートで円換算（既存の通貨換算機能）。
  「Not available」「0.00」は価格なし(null)扱い。
- **数量（quantity）＝1台あたりの使用員数**。元サイトのカート初期値がこれに相当。全パーツに反映する。
  同一品番でもユニット（図面）により員数が異なるため、必ずユニット単位で反映する。
- 同一図番号に取り消し線の無い複数品番（色違い/メーカー違い/現行併売）は**すべて別行で登録**する。
- **展開図はサイトの元画像から透かし除去＋白黒化**して Cloudinary に保存し、各ユニットに1枚紐付け。
- 取得時は**低速・1件ずつ・同時アクセスなし**（サイト保護／bot判定回避）を厳守する。
- データ整合性: `part(genre経路)=part(unit経路)`（unitId=null の孤児が無いこと）を検算する。

### UI要件（一覧カードの自動調整・2026-09-17 実装済み）
- ジャンル/ユニット/カテゴリーの一覧カードは、名前が長くても**枠内に自動で収める**
  （カード高さは最小高さで必要に応じ伸長、フォント調整＋折り返し）。新規カテゴリーでも自動適用。

### 関連ドキュメント（設計・進捗の所在）
- 全体ロードマップ・方針: `docs/PRODUCT_ROADMAP.md`
- マルチテナント詳細設計＋見積り: `docs/PHASE3_TENANT_DESIGN.md`
- 進捗: `docs/SCOPE_PROGRESS.md`

---

**作成日**: 2025-11-13
**バージョン**: 1.1.0（2026-09-18 FK2 LH/RHデータ取込・数量・UI自動調整の要件を追補 / 2026-09-15 販売・マルチテナント要件を追補）
