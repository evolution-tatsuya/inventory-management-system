...
## 9. 決定事項（2026-09-19 確定）＋残課題

### 確定
- **テナント識別 = URLパスプレフィックス（/t/顧客A）**。
  理由: 他テナントの管理画面に誤って入る事故を構造的に防ぐ（=大クレーム源）。
  実装: フロントのルーティングに `/t/:slug` を前置。認可は URL の slug と JWT の tenantId の
  一致を requireAuth で強制し、不一致は 403。全 Prisma クエリは JWT 由来の tenantId でスコープ。
- **master ログイン = 別導線**。運営者専用ログイン → 総括ページ。顧客 admin/user とは画面・URLを分離。
- **課金/サブスク = スコープに含めるが、決済は別構築中のECサイトで実施**。
  在庫アプリに決済UIは持たない。EC側の「契約成立 / 解約 / 支払い失敗」を受けて
  Tenant.status（active/suspended）を切り替える**連携**として実装する。
  → 在庫アプリ側の課金責務は「テナント発行・有効化・停止のAPI」に限定され、スコープはむしろ軽い。

### 残課題（EC仕様確認後に確定）
- **EC⇔在庫アプリ の連携方式**: Webhook（EC→在庫のAPIを叩く）/ ポーリング / 手動 のどれか。
- **テナント識別キー**: EC側の顧客ID / メール / 契約ID のどれで Tenant を紐づけるか。
- **状態マッピング**: ECのサブスク状態（active/canceled/past_due 等）→ Tenant.status への対応表。
- 専用インスタンス発行の自動化レベル（S5。当面は手動デプロイ許容）。

※ ECは既に本番稼働中（動作確認しながら修正中）。連携インターフェースは実物を確認して設計する。
