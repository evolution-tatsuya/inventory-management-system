...
## 📌 プロダクト計画（2026-09-14 追記）

販売・複製・在庫独立化のロードマップは **docs/PRODUCT_ROADMAP.md** に記載。

- フェーズ2（着手中）: 1アプリ内の完成
  - ② 在庫モード切替式（shared / perCategory）
  - ③ 運営者向け総括ページ（在庫モード切替のシンプル版）
  - ① GTR-GT3 Rev.4.1 を GT3-049 から独立複製（完了条件: カテゴリー間で在庫がかぶらない）
- フェーズ3（今後計画）: 販売用にアプリ丸ごと複製（マルチテナント or インスタンス複製）＋総括ページ本格化

---

## 🧭 設計判断ログ（フェーズ3 / 2026-09-15）

後から経緯を追えるよう、意思決定を時系列で記録。

1. **在庫独立化 完了（フェーズ2）**: shared/perCategory 切替式を実装。運営者ページ(/admin/owner)で切替。
   GTR-GT3 Rev.4.1 を GT3-049 から独立複製（中身完全一致・在庫独立）。本番反映済み。
2. **販売方式の検討**: マルチテナント型 vs インスタンス複製型を比較（比較資料あり）。
   → **「いいとこ取り」に決定**（コードはテナント対応で1回修正・全反映／デプロイは共有・専用を選択）。
3. **販売モデル確定**: 空納品＋オプションでデータ代行入力（有償）。原本の全顧客配信機構は不要。
   → ユーザー懸念「データ修正が他納品先に影響」は tenant分離＋空納品で構造的に解消。
4. **詳細設計＋見積り作成**: `docs/PHASE3_TENANT_DESIGN.md`。
   影響範囲=Prismaクエリ82箇所、全10テーブルtenantId。5段階(S1移行/S2認証/S3全クエリ/S4総括/S5専用デプロイ)。
   最大リスク=S3の付け忘れ漏洩 → Prisma拡張でtenantId自動注入を先に入れる方針。

### ✅ 着手前の要決定事項（2026-09-19 確定）
- **テナント識別方法: URLパス方式（/t/顧客A）で確定**。
  理由（ユーザー）=「間違って別ユーザーになるのはまだしも、他人の管理画面に入れたら大クレーム」。
  URLで明示分離し、認可（自 tenantId 以外は全クエリで拒否）を厳格化する。
- **master のログイン導線: 別導線で確定**（運営者専用ログイン→総括ページ）。顧客管理者と完全分離。
- 専用インスタンス発行の自動化レベル: 未確定（S5で詰める。当面は手動デプロイ許容）。
- **課金/サブスク: スコープに含める。ただし決済は別途構築中のECサイトで行う**。
  在庫アプリ側に決済UIは作らない。EC側の契約成立/解約を受けて在庫アプリの Tenant.status を
  active/suspended に切り替える**連携**として実装。連携方式（Webhook/API/手動）はEC側仕様を
  確認して確定する（ECは既に本番稼働・動作確認しながら修正中とのこと。URL共有待ち）。

### 現在地
- フェーズ2 完了・本番反映済み。フェーズ3 は **設計完了・実装未着手**。
- 要決定4項目のうち3項目確定（識別=パス / master=別導線 / 課金=EC連携）。
  残1=EC連携方式（EC仕様の確認後に確定）。
- 次アクション: EC連携インターフェースを確認 → S1（tenantId導入）に着手。
